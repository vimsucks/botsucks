...
class CustHandler(Handler):
    APISUCKS_SCORE_URL_FORMAT = 'https://api.vimsucks.com:8000/cust/student/%s/score'

    def __init__(self):
        super().__init__(command="cust", pass_args=True)

    def check_score_update(self, bot: telegram.Bot, student):
        resp = requests.post(CustHandler.APISUCKS_SCORE_URL_FORMAT % student.sid, {
    ...
    def command_login(self, bot: telegram.Bot, update: telegram.Update, args):
        sid, password = args[1], args[2]
        resp = requests.post(CustHandler.APISUCKS_SCORE_URL_FORMAT % sid, {
            'password': password,
        })
        if resp.status_code == HTTPStatus.OK:
            ...
            self.check_score_update(bot, student)

    def command_logout(self, bot: telegram.Bot, update: telegram.Update, args):
        try:
            user = User.select().where(User.id == update.message.from_user.id).first()
    ...
    def command_score(self, bot: telegram.Bot, update: telegram.Update, args):
        user = User.select().where(User.id == update.message.from_user.id).first()
        student = None
        try:
            student = CustStudent.get(user=user)
    ...
    def handle(self, bot: telegram.Bot, update: telegram.Update, args):
        ...
        if sub_command == "login":
            self.command_login(bot, update, args)
        elif sub_command == "logout":
            self.command_logout(bot, update, args)
        elif sub_command == 'score':
            self.command_score(bot, update, args)


class CustJob(RepeatingJob):

    def __init__(self):
        super().__init__(60 * 30, 0)

    def callback(self, bot, job):
        students = CustStudent.select()
        for student in students:
            CustHandler().check_score_update(bot, student)
